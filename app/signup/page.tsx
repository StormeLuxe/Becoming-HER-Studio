    ...
    <main style={{ minHeight: "100vh", background: "radial-gradient(ellipse 60% 40% at 0% 0%, rgba(255,45,166,0.12) 0%, transparent 50%), radial-gradient(ellipse 50% 40% at 100% 0%, rgba(147,51,234,0.12) 0%, transparent 50%), radial-gradient(ellipse 40% 30% at 50% 100%, rgba(255,45,166,0.08) 0%, transparent 50%), #12031E", display: "flex", alignItems: "center", justifyContent: "center", padding: "1.5rem", fontFamily: "'DM Mono', monospace" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, width: "100%", maxWidth: 420, textAlign: "center" }}>
        <div style={{ marginBottom: "2.5rem" }}>
          ...
          <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "1rem", fontStyle: "italic", color: "#8b6faa" }}>Begin your becoming.</p>
        </div>

        {sent ? (
          <div>
            <p style={{ fontFamily: "'Playfair Display', serif", fontStyle: "italic", fontSize: "1.1rem", color: "#f0e6ff", marginBottom: "0.75rem" }}>Check your inbox.</p>
            <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "0.95rem", color: "#8b6faa", lineHeight: 1.7 }}>
              A confirmation link is on its way to <span style={{ color: "#c77dff" }}>{email}</span>.<br />Click it to activate your studio.
            </p>
            <p style={{ marginTop: "2rem", fontFamily: "'Cormorant Garamond', serif", fontStyle: "italic", fontSize: "0.9rem", color: "#8b6faa" }}>
              Already confirmed? <Link href="/login" style={{ color: "#c77dff", textDecoration: "none" }}>Sign in</Link>
            </p>
          </div>
        ) : (
          <>
            <form onSubmit={handleSignup} style={{ textAlign: "left" }}>
              ...
              <button type="submit" disabled={loading || !email || password.length < 8} style={{ width: "100%", padding: "1rem 2rem", background: loading || !email || password.length < 8 ? "rgba(123,47,255,0.2)" : "linear-gradient(135deg, #FF2DA6, #f72585)", border: "none", borderRadius: 4, color: "#fff", fontFamily: "'DM Mono', monospace", fontSize: "0.65rem", letterSpacing: "0.25em", textTransform: "uppercase", cursor: loading || !email || password.length < 8 ? "not-allowed" : "pointer", marginTop: "0.5rem", opacity: loading || !email || password.length < 8 ? 0.5 : 1, boxShadow: loading || !email || password.length < 8 ? "none" : "0 4px 15px rgba(255, 45, 166, 0.3)", transition: "all 0.3s ease" }}>
                {loading ? "Creatingâ€¦" : "Create My Account"}
              </button>
            </form>
            <p style={{ marginTop: "2rem", fontFamily: "'Cormorant Garamond', serif", fontStyle: "italic", fontSize: "0.9rem", color: "#8b6faa" }}>
              Already have an account? <Link href="/login" style={{ color: "#c77dff", textDecoration: "none" }}>Sign in</Link>
            </p>
          </>
        )}
      </div>
